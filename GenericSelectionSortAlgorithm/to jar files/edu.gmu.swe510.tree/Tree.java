 ...
 * @author Alex Lawson
 *
 *Code is modified from the tree application from the book
 *
 * @param <E>
 */
...
//the two overloaded search methods
   ...
   private boolean search(TreeNode node, E searchValue)
   {
		if (node == null) {
			return false;
		}
		else if (searchValue.compareTo((E) node.data) == 0) {
			return true;
		}
		else if (searchValue.compareTo((E) node.data) < 0) {
			return search(node.leftNode, searchValue);
		}
		else {
			return search(node.rightNode, searchValue);
		}
   }
